...
export default function App() {
  ...
  const isMainScreen = (screen) => screen === 'profile' || screen === 'agenda';

  useEffect(() => {
    if (!isAuthReady) return;

    if (authUser) {
      setCurrentScreen((prev) => (isMainScreen(prev) ? prev : 'profile'));
      return;
    }

    setCurrentScreen((screen) => (isMainScreen(screen) ? 'home' : screen));
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {currentScreen === 'agenda' && authUser ? (
        <AgendaScreen authUser={authUser} />
      ) : null}
      {authUser && isMainScreen(currentScreen) ? (
        <View style={styles.tabBar}>
          <Pressable
            style={[styles.tabItem, currentScreen === 'profile' && styles.tabItemActive]}
            onPress={() => setCurrentScreen('profile')}
          >
            <FontAwesome name="user" size={22} color={currentScreen === 'profile' ? '#0f172a' : '#94a3b8'} />
            <Text style={[styles.tabLabel, currentScreen === 'profile' && styles.tabLabelActive]}>Profile</Text>
          </Pressable>
          <Pressable
            style={[styles.tabItem, currentScreen === 'agenda' && styles.tabItemActive]}
            onPress={() => setCurrentScreen('agenda')}
          >
            <FontAwesome name="calendar" size={22} color={currentScreen === 'agenda' ? '#0f172a' : '#94a3b8'} />
            <Text style={[styles.tabLabel, currentScreen === 'agenda' && styles.tabLabelActive]}>Agenda</Text>
          </Pressable>
        </View>
      ) : null}
      <StatusBar style="auto" />
